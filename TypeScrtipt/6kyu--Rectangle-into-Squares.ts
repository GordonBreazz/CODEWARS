/***************************************************************************************************************************************************************************************************************************************
 *
 *     KATA: 6 kyu "Rectangle into Squares"
 *
 *   SOURCE: https://www.codewars.com/kata/55466989aeecab5aac00003e
 *   AUTHOR: https://www.codewars.com/users/GordonBreazz
 *
 *     PLOT: The drawing below gives an idea of how to cut a given "true"
...
function search(minl, maxl, list) {
  let s: number = minl * maxl
  if (s <= 0) return
  if (s <= 1) {
    list.push(1)
    return list
  ...
  let x: number = Number(Math.floor(Math.sqrt(s)))
  if (x > minl) x = minl
  list.push(x)

  search(Math.min(x, maxl - x), Math.max(x, maxl - x), list)
  return list
}
...
export class G964 {
  public static sqInRect(l: number, w: number): number[] {
    ...
    if (l == w) return null
    let arr: number[] = []
    return search(Math.min(l, w), Math.max(l, w), arr)
  }
}
